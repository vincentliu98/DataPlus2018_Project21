...
ui <- fluidPage(
  # App title ----
  titlePanel("Content-Based Recommender"),
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      # Create a text input box for Programs
      #textInput("programs", label = h3("Co-Curricular Programs"), 
                #value = "Enter co-curricular programs..."),
      # Include clarifying text ----
      helpText("Note: From the drop-down menu, select all of the co-curricular 
                programs you have participated in during your time at Duke.") 
    ),
    mainPanel(
  ...
  hr(),
  fluidRow(
    verbatimTextOutput('user_programs')
  )
)
...
server <- function(input, output) {
  ...
  # For text input --> output$user_programs <- renderPrint({strsplit(input$programs, ", ")})
